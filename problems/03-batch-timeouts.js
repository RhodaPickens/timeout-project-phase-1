...
function batchTimeouts(callbacks, delays) {
  ...
  // iterate through parameters to access indices
  for (let i = 0; i < callbacks.length; i++) {

    // call setTimeout with the same index for callback and delay
    const val = setTimeout(callbacks[i], delays[i]);
    // make an array of individual timeout objects
    objects.push(val);
  }
